...
export default function Nav({ show }) {
  const inactiveLink = 'flex gap-2 p-2 pr-0';
  const activeLink = inactiveLink + 'bg-white text-blue-900 bg-white rounded-l-lg pr-0 font-bold';
  const inactiveIcon = 'w-8 h-6';
  const activeIcon = inactiveIcon + ' text-blue-900';
  ...
  return (
    <aside className={(show ? 'left-0' : '-left-full') + " top-0 text-white flex flex-col gap-5 py-3 w-full bg-blue-900 h-full md:h-screen md:w-auto transition-all z-50 pb-[291px] sticky md:pb-0 md:pt-0 md:sticky"}>
      <div className="flex gap-2 mb-5 mr-5 p-5">
        <Logo />
      </div>
